...
export class AppComponent {
  ...
  //custom function
  setActiveColor(styleName, value) {
    var imgColor = {};
    imgColor[styleName] = value;
    console.log("'"+imgColor[styleName]+"'");
    return imgColor[styleName];
  }
  setFilterColor(styleName, value) {
    var imgColor = {};
    var unit = (<HTMLInputElement>document.getElementById("test")).value;
    imgColor[styleName] = value;
    let picVal = "'"+imgColor[styleName]+"'";
    let object = this.canvas.getActiveObject();
    if (!object) return;
    console.log(unit);
    let filter = new fabric.Image.filters.Tint({
      color: unit // make it red!
      // mode: 'tint',
      // opacity: 0
      // brightness: 0.05
    });
  ...
  setFillImage() {
   return  this.setActiveColor('fill', this.props.fill);
  }
  settestImage() {
    this.setFilterColor('imgfill', this.props.imgfill);
  }
